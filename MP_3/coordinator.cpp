...
public:
  Status HandleHeartBeats(ServerContext *context, ServerReaderWriter<Heartbeat, Heartbeat> *stream) override
  {
    ...
    std::thread reader_thread([&]()
            ...
            while (stream->Read(&h))
            {
              string type = (h.server_type() == ServerType::SLAVE)? "slave" : "master";
      //        cout  << "heartbeat" << endl;
                log(INFO, "heartbeat recieved from " << type << " " << id);
    ...
    while (true)
    {
      ...
      {
        string type = (h.server_type() == ServerType::SLAVE) ? "slave" : "master";
        log(INFO, "Two heartbeats not recieved from " << type << " " << id);
  ...
  Status GetServer(ServerContext *context, const User *user, Server *server) override
  {
   ...
   //      cout << "checkpoint" << endl;
    ...
    {
      // cout << "yo3" << endl;
      log(INFO, "Master Server" << id << " returned for user " << user->user_id());
  ...
  Status GetSlave(ServerContext *context, const ClusterId *cluster_id, Server *server) override
  {
    ...
    {
      log(INFO, "GetSlave pending for cluster  " << cluster_id->cluster());
...
void RunServer(std::string port_no)
{
  ...
  srv->Wait();
  log(INFO, "Coordinator started up, ready to listen");
  // cout << "hey" << endl;
}
